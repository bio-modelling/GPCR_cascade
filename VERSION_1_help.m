function varargout = VERSION_1_help(varargin)
% VERSION_1_HELP M-file for VERSION_1_help.fig
%      VERSION_1_HELP, by itself, creates a new VERSION_1_HELP or raises the existing
%      singleton*.
%
%      H = VERSION_1_HELP returns the handle to a new VERSION_1_HELP or the handle to
%      the existing singleton*.
%
%      VERSION_1_HELP('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in VERSION_1_HELP.M with the given input arguments.
%
%      VERSION_1_HELP('Property','Value',...) creates a new VERSION_1_HELP or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before VERSION_1_help_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to VERSION_1_help_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help VERSION_1_help

% Last Modified by GUIDE v2.5 19-Oct-2009 16:33:40

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @VERSION_1_help_OpeningFcn, ...
                   'gui_OutputFcn',  @VERSION_1_help_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before VERSION_1_help is made visible.
function VERSION_1_help_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to VERSION_1_help (see VARARGIN)

% Choose default command line output for VERSION_1_help
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

%This globalises all of the variables in the GUI
%{
for i=1:70
    handlestring=['handles.text',int2str(i)];
    handle=eval(handlestring);
    namestring=get(handle, 'String');
    %global_namestring=['global ',namestring];
    %eval(global_namestring)
    eval(namestring);
end
%}

% UIWAIT makes VERSION_1_help wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = VERSION_1_help_OutputFcn(hObject, eventdata, handles) 
...
if (get(handles.stochbutton,'Value') == get(handles.stochbutton,'Max'))
	param_set(1)={'flagM=1'};
    param_set(2)={'flagG=1'};
    param_set(3)={'flagP=1'};
    param_set(4)={'flagD=1'};
...
for j=1:73
    ...
    name_tag_string=['handles.text',int2str(j)];
    name_tag=eval(name_tag_string);
    name_string=get(name_tag,'String');
    
    value_tag_string=['handles.edit',int2str(j)];
    value_tag=eval(value_tag_string);
    value_string=get(value_tag,'String');
    
    if j==56
    parameter_set_string=[name_string,'=',value_string,'e-18'];
    else
    parameter_set_string=[name_string,'=',value_string];
    end
    param_set(j+5)={parameter_set_string};
    
end

[t I M Gd GPLCd PIP2loss DAGd Ntrp Nact Ca Cafree]= gui_singleQB(param_set);

eval(param_set{78});
Ifilter = Filter(I,t,fcut);

figure(1)
hold on
plot(DAGd,Ntrp,'black')
...
subplot(6,1,5), plot(t,Ifilter,'b'); %,t,I,'b')
...
for k=1:4
    ...
    name_tag_string=['handles.textm',int2str(k)];
    name_tag=eval(name_tag_string);
    name_string=get(name_tag,'String');
    
    value_tag_string=['handles.editm',int2str(k)];
    value_tag=eval(value_tag_string);
    value_string=get(value_tag,'String');
    
    parameter_set_string=[name_string,'=',value_string];

    m_set(k)={parameter_set_string};
end

procflag=0;

if (get(handles.twoprocbutton,'Value') == get(handles.twoprocbutton,'Max'))
    procflag=2;
end

if (get(handles.fourprocbutton,'Value') == get(handles.fourprocbutton,'Max'))
    procflag=4;
end

gui_averageQBall(param_set, m_set, procflag);
...
% --- Otherwise, executes on mouse press in 5 pixel border or over edit56.
function edit56_ButtonDownFcn(hObject, eventdata, handles)
% hObject    handle to edit56 (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
